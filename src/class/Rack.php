...
class Rack {	
	...
	public function listRack(){		
		...
		while ($rack = $result->fetch_assoc()) { 				
			...
			$rows[] = $rack['status'];							
			$rows[] = '<button type="button" name="update" id="'.$rack["rackid"].'" class="btn btn-warning btn-xs update"><span class="glyphicon glyphicon-edit" title="Edit">Edit</span></button>';
			$rows[] = '<button type="button" name="delete" id="'.$rack["rackid"].'" class="btn btn-danger btn-xs delete" ><span class="glyphicon glyphicon-remove" title="Delete">Delete</span></button>';
			$records[] = $rows;
			$count++;
		}
